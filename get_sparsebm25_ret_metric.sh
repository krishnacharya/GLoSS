...
PYTHON_SCRIPT="/home/kacharya33/nanoGenRec/src/calcirmetrics_bm25_dataset_agno.py"
...
run_job() {
  ...
  local split="$5"
  # local encoder_name="$6" # This parameter is not in the example JSON args, so we remove it from the function signature.
  local short_model_name="$6" # Shifted to $6 after removing encoder_name
...
run_job "beauty" "amazon" "llama-8b/llama-8b-8.3k_test_beam5_max_seq1024_bs8_numret5.json" "$DEFAULT_NUM_SEQUENCES" "test" "llama-8b" || exit 1

### Toys dataset
run_job "toys" "amazon" "llama-1b/Llama-3.2-1B-fttoys_ep10_maxseq1024_bs4_acc4_test_beam5_max_seq1024_bs8_numret5.json" "$DEFAULT_NUM_SEQUENCES" "test" "llama-1b" || exit 1
