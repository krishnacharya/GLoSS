...
def gen_with_beam(model_name: str, dataset_name: str, dataset_split: str,  max_seq_length:int,
                  batch_size: int, num_beams: int, num_return_sequences: int, max_new_tokens: int=50, data_field:str="ptext"):
    ...
    gen_path = str(get_gen_dir_dataset(dataset_name) / f"{model_name.split('/')[-1]}_{dataset_split}_beam{num_beams}_max_seq{max_seq_length}.json")
    with open(gen_path, "w") as f:
        json.dump(results, f, indent=2)

if __name__ == "__main__":
    # model_name = "Llama-3.2-1B-ftbeauty2014"
    model_name = "checkpoint-9700-cop"
    dataset_name = 'beauty2014'
    num_beams = 5
    num_return_sequences = 5
    gen_with_beam(model_name = model_name, dataset_name = dataset_name, dataset_split = 'test', max_seq_length = 1024,
                  batch_size = 8, num_beams = num_beams, num_return_sequences = num_return_sequences, max_new_tokens = 50, data_field = 'ptext')
