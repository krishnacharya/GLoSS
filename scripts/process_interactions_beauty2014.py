...
def load_and_preprocess_reviews(file_path):
    """Loads and preprocesses the beauty reviews dataset."""
    beauty_5 = pd.read_json(file_path, lines=True)
    cols = ['reviewerID', 'asin', 'unixReviewTime', 'overall']
    beauty_5 = beauty_5[cols]
    return beauty_5
...
def visualize_reviewer_counts(df):
    ...
    plt.title('Histogram of Review Counts per Reviewer')
...
def main():
    ...
    os.system(f"wget https://snap.stanford.edu/data/amazon/productGraph/categoryFiles/reviews_Beauty_5.json.gz -P {str(raw_dir2014)}")
    os.system(f"gunzip {os.path.join(str(raw_dir2014), 'reviews_Beauty_5.json.gz')}")
    reviews_file = str(raw_dir2014 / 'reviews_Beauty_5.json')
    metadata_file = str(processed_data_dir('beauty2014') / 'meta_proc.json')

    # Load and preprocess reviews
    beauty_df = load_and_preprocess_reviews(reviews_file)

    # Deduplicate reviews
    df_withdup, df_dedup = deduplicate_reviews(beauty_df)

    # Visualize reviewer counts
    visualize_reviewer_counts(df_dedup)
    ...
    try:
        ...
        print(f"Error: Metadata file '{metadata_file}' not found. Please ensure the path is correct.")
        return
    ...
    df_withdup.to_json(str(processed_data_dir('beauty2014') / 'df_withdup.json'), orient='records', lines=True)
    df_dedup.to_json(str(processed_data_dir('beauty2014') / 'df_dedup.json'), orient='records', lines=True)
    ...
    create_corpus_metadata(meta_df, unique_asins_withdup, output_path=str(processed_data_dir('beauty2014') / 'meta_corpus.json'))
