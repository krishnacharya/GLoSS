...
def textbased_lastsimilar(dataset: Dataset, retriever: bm25s.BM25, item_id_to_nlang: Dict[str, str], items_compact: pd.DataFrame, config: Dict, k: int = 5) -> Dict[str, Dict[str, float]]:
    """
    ...
    for i in tqdm(range(l), desc="Generating Recommendations"):
        user_id = dataset[user_id_key][i]  # Use user_id_key
        item_indices = res[i]
        item_scores = scores[i]
        items = items_compact.iloc[item_indices][item_id_key].tolist()  # Use item_id_key
