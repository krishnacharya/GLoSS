...
def generate_beam_sequences(model, tokenizer, dataset: Dataset,
    ...
    for i in tqdm(range(0, dataset_size, batch_size), desc="Generating beam sequences", unit="batch"):
        ...
        for j, input_text in enumerate(input_texts): # input_text is not used below, but enumerate is useful
            ...
            results.append({
                "user_id": dataset[original_dataset_index]['user_id'],
                "movie_id": dataset[original_dataset_index]['movie_id'], # target movie id
                "seen_movie_ids": dataset[original_dataset_index]['seen_movie_ids'],
...
def gen_with_beam(model_name: str, dataset_name: str, dataset_split: str,  max_seq_length:int,
    ...
    run_name = f"gen_bs{batch_size}_numret{num_return_sequences}_maxlen{max_new_tokens}_{model_name.split('/')[-1]}_{dataset_name}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, required=True, help="Name/path of the language model.")
    parser.add_argument("--dataset_name", type=str, required=True, help="Name of the Hugging Face dataset directory (e.g., 'beauty2014').")
    parser.add_argument("--dataset_split", type=str, default="validation", help="Split of the dataset to use (e.g., 'train', 'validation').")
    parser.add_argument("--max_seq_length", type=int, default=1024, help="Maximum sequence length for input prompts.")
    ...
    parser.add_argument("--data_field", type=str, default="ptext", help="Name of the dataset column containing the input text.")
