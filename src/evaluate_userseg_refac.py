...
def analyze_user_group_metrics(dataset, model_name, encoder_name, cold_users_set, regular_users_set, power_users_set, metrics_cols=['recall@5', 'ndcg@5']):
    ...
    # Determine the base directory for per-user metrics
    # HACKY: Adjust path construction for LIS metrics
    if encoder_name.startswith('LIS/'):
        # For LIS metrics, the path structure is 'peruser_metrics/LIS/{encoder_subfolder}/{dataset}.jsonl'
        # The model_name is not part of the directory structure here.
        lis_encoder_subfolder = encoder_name.split('/')[1] # e.g., 'bm25' or 'intfloat'
        
        # Further refine for 'intfloat' which has e5-base-v2 or e5-small-v2
        if lis_encoder_subfolder == 'intfloat':
            # Example: peruser_metrics/LIS/intfloat/e5-base-v2/beauty.jsonl
            # The 'encoder_name' passed here could be 'LIS/intfloat/e5-base-v2'
            # We need to extract 'e5-base-v2' from it.
            intfloat_model_type = encoder_name.split('/')[2] 
            dir_path = get_peruser_metric_dataset_modelname_encoder(
                dataset="", # Not used in this specific LIS pathing
                model_name="", # Not used in this specific LIS pathing
                encoder_name=f"LIS/{lis_encoder_subfolder}/{intfloat_model_type}"
            )
            file_name = f"{dataset}.jsonl"
        else: # For 'bm25s' or 'bm25'
            dir_path = get_peruser_metric_dataset_modelname_encoder(
                dataset="", # Not used in this specific LIS pathing
                model_name="", # Not used in this specific LIS pathing
                encoder_name=f"LIS/{lis_encoder_subfolder}"
            )
            file_name = f"{dataset}.jsonl"
            
    else:
        # Original path construction for non-LIS models
    ...
    # For LIS data, the user ID column might be different, commonly 'user_id'
    user_id_col = 'reviewer_id' if 'reviewer_id' in pum_df.columns else 'user_id'
    
    if user_id_col not in pum_df.columns:
        print(f"Error: Neither 'reviewer_id' nor 'user_id' column found in the metrics file for {dataset}. Please check the data format.")
        return
    ...
    cold_start_metrics_df = pum_df[pum_df[user_id_col].isin(cold_users_set)]
    regular_metrics_df = pum_df[pum_df[user_id_col].isin(regular_users_set)]
    power_metrics_df = pum_df[pum_df[user_id_col].isin(power_users_set)]
...
if __name__ == "__main__":
    ...
    # Define models and encoders to evaluate
    cols_to_analyze = ['recall@5', 'ndcg@5']
    
    # Original models and encoders
    models_to_evaluate_original = ['llama-1b', 'llama-3b', 'llama-8b']
    encoders_to_evaluate_original = [
        'bm25s',
        'intfloat/e5-small-v2',
        'intfloat/e5-base-v2'
    ]

    # LIS specific configurations
    # For LIS, the 'model_name' in analyze_user_group_metrics will effectively be a placeholder,
    # as it's not used in the LIS file path construction.
    lis_models = ['LIS'] # Use 'LIS' as a conceptual model for LIS metrics
    lis_encoders = [
        'LIS/bm25', # The subfolder is 'bm25' for LIS
        'LIS/intfloat/e5-base-v2',
        'LIS/intfloat/e5-small-v2'
    ]

    # Analyze metrics for each specified dataset and model/encoder combination
    for target_dataset in datasets_config.keys():
        if target_dataset in user_groups:
            # --- Original models and encoders ---
            for encoder_name in encoders_to_evaluate_original:
                for model_name in models_to_evaluate_original:
            ...
            for lis_encoder in lis_encoders:
                # For LIS, model_name is not relevant in the filename, using 'LIS' as a placeholder
                analyze_user_group_metrics(
                    dataset=target_dataset,
                    model_name='LIS_model', # Placeholder, won't be used in path
                    encoder_name=lis_encoder,
                    cold_users_set=user_groups[target_dataset]['cold'],
                    regular_users_set=user_groups[target_dataset]['regular'],
                    power_users_set=user_groups[target_dataset]['power'],
                    metrics_cols=cols_to_analyze
